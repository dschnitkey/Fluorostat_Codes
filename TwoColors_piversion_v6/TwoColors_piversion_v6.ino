...
int target_OD = 280; //OD voltage set point for turbidostat 
...
//LD Variables---------------
long delaytime = ((6L * 60L + 40L) * 1000L * 60L); //num mins to night   // maybe change later
...
void turbidostat_cyano_air_LD(int targetOD, long stime, long dtime, int count) {
  ...
  //Serial.println(timeleft);
  if (timeleft < 1000) { // A tolerance of 1000ms might be unnecessarily tight 
    //starttime = millis();
    starttime += delaytime;
    LightsOn = !LightsOn;
    ...
    if (LightsOn) {
      LevelLED_OFF();
      transtime = daylength; // There is probably a better way to do this but here is a temporary fix - AFS 23/04/25
    }
    else {
      LevelLED_ON();
      transtime = nightlength;
  ...
  temperature_measurements_and_set_peltier();
  wait(1);
  if (counter < 3 and LastODRead < targetOD) {
    turbido_pump(10, targetOD); // 2 secs pump in, in case of overflow
    // re measure the OD and ensure you got below the threshold
    LevelLED_ON();//Lights OFF
    OD_signal_read();
    if (LightsOn) {
      LevelLED_OFF();//Lights ON
    }
    if (LastODRead < targetOD) {
      counter = counter + 1;
    }
    else {
      counter = 0;
  ...
  AirPin_ON();
  // AFS 22/11/17: I removed the if (LightsOn) statement here because they should turn on after the final 
  // OD reading during turbidostat dilution. If the OD hasn't crossed the threshold then they were just turned on
  // by the boolean check if (LightsOn) after the regular call of OD_signal_read();
  for (int i = 0; i < 180; i++) { //15 minutes between readings
    wait(5);
    temperature_measurements_and_set_peltier();
    //AirPin_ON();
...
//
  // Light test//
//  LevelLED_OFF();    // OFF means lights on
//  wait(5);
//  AirPin_OFF();
//  LevelLED_ON();
//  wait(5);
...
//    OD_signal_read();
  ...
  //pump event
//    pump_out(10); // in case the water level too high
//    PumpIn_ON();
//  
//    
//
